...
class AdvancedRecipeResolver:
    ...
    def unwrap_recipe_data(self, data):
        """Unwraps JSON formats like {'minecraft:recipe_shaped': {...}} to get the inner recipe."""
        if not isinstance(data, dict):
            return None, ""

        # Check for nested keys like 'minecraft:recipe_shaped', 'minecraft:recipe_furnace', etc.
        for key, value in data.items():
            if "recipe" in key and isinstance(value, dict):
                return value, key

        # Return standard top-level JSON recipe data
    ...
    def parse_json_file(self, file_path):
        try:
            ...
            # 1. Shaped Crafting (Handles key maps like '#', 'X')
            ...
            # 3. AE2 Transform / World Processing
            ...
            # 4. Standard ingredient arrays/objects (Furnace, Smelting, Shapeless)
