...
class AdvancedRecipeResolver:
    def __init__(self):
        ...
        self.item_caps = {}  # {item_id: max_craftable_qty}
    ...
    def normalize_id(self, item_id):
        ...
        if len(parts) == 2:
            ...
            if ns in self.namespace_mappings:
                s_id = f"{self.namespace_mappings[ns]}:{path}"
        elif not s_id.startswith("minecraft:"):
            s_id = f"minecraft:{s_id}"

        return s_id

    def normalize_tag(self, tag_str):
        """Formalizes tag strings by stripping extraneous hashes and mapping namespaces."""
        clean_tag = str(tag_str).strip().lstrip("#")
        parts = clean_tag.split(":", 1)
        
        if len(parts) == 2:
            ns, path = parts
            path = path.replace("/", "_")  # Normalize slash paths to underscores
            if ns in self.namespace_mappings:
                ns = self.namespace_mappings[ns]
            clean_tag = f"{ns}:{path}"
        else:
            clean_tag = f"tag:{clean_tag.replace('/', '_')}"
